...
test('resolve with npm package manager puts install in setup', t => {
...
test('resolve with npm has run cmd as just the script', t => {
  const result = nodeKit.resolve({script: 'app.js', packageManager: 'npm'})
...
test('resolve with install=false skips setup', t => {
  const result = nodeKit.resolve({script: 'app.js', install: false})
  t.is(result.setup, undefined)
  t.deepEqual(result.cmd, ['sh', '-c', 'node /app/app.js'])
})

test('resolve with install has no top-level caches or allowNetwork', t => {
  const result = nodeKit.resolve({script: 'app.js'})
...
test('resolve with run still creates setup by default', t => {
...
test('resolve with run and install=false skips setup', t => {
