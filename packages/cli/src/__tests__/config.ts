...
function tempDir(): string {
  return join(tmpdir(), `pipex-test-${randomUUID()}`)
}

test('loadConfig returns {} when no .pipex.yml', async t => {
  const dir = tempDir()
...
test('loadConfig returns {} for empty file', async t => {
  const dir = tempDir()
  await mkdir(dir, {recursive: true})
  await writeFile(join(dir, '.pipex.yml'), '', 'utf8')
  const config = await loadConfig(dir)
  t.deepEqual(config, {})
})
...
test('loadConfig throws on invalid YAML', async t => {
  const dir = tempDir()
  await mkdir(dir, {recursive: true})
  await writeFile(join(dir, '.pipex.yml'), ':\n  - :\n    bad: [', 'utf8')
